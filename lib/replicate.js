...
var randomNumber = require('./deps/randomNumber');

var MAX_SIMULTANEOUS_REVS = 50;
function standOff(repId, src, target, opts, returnValue, result, error) {
  if (opts.retry === false) {
  ...
  opts.retries++;
  if (opts.maxRetries && opts.retries > opts.maxRetries) {
  ...
  var min = opts.standOff || 0;
  var max = 0;
  if (!min) {
    max = 2000;
  }
  randomNumber(min, max).then(function (time) {
    opts.standOff = time;
    setTimeout(function () {
...
function replicate(repId, src, target, opts, returnValue, result) {
  ...
  function completeReplication() {
    ...
    if (non403s.length > 0) {
      ...
      error.result = result;
      standOff(repId, src, target, opts, returnValue, result, error);
    } else {
